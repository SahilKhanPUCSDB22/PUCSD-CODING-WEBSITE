...
static enum erno send_session(unsigned char nonce[],int sockfd)
{
	if(sodium_init()==-1)
	{
	...
	if( send(sockfd,(void*)nonce,Len,0) < 0)
	{
...
static enum erno send_page(char *input , unsigned char *nonce,int sockfd)
{
	...
	fprintf(fp,"required><br>\n<input type=\"text\" id=\"xyz\" name=\"id\" value=\"%s\">\n<button> Submit </button><br>\n</form>\n<p><a href=\"/cgi-bin/upload.cgi?userid=%s\">Click here to Upload Files</a></p>\n<p><a href=\"/cgi-bin/viewup.cgi?userid=%s\">Click here to view Uploaded Files</a></p>\n<p><a href=\"/login.html\">Click here to Logout</a></p>\n</body>\n</html>",input,input,input);
...
int main(void)
{
	...
	while(loop!=0)
	{
		...
		if(cfd!=-1)
		{
			...
			if((flow==0) && (send_session(nonce,cfd)!=SESSION_OK)  )
			{
				send_error(DATA_F,cfd);
				flow=1;	
			}

			if( (flow==0) && (update_session((const char*)nonce)!=UPDATE_OK))
			{
				send_error(DATA_F,cfd);
				flow=1;
			}
